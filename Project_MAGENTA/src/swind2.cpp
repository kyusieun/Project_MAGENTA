#include "../include/dseFix/global.h"
#include <shlwapi.h>
#include <devioctl.h>
#include <strsafe.h>
...
)
{
	*CiOptionsAddress = nullptr;
	...
	const CHAR* selectedDll = (NtCurrentPeb()->OSBuildNumber >= 9200) ? CiDll : NtoskrnlExe;

	WCHAR DllPath[MAX_PATH];
	int requiredSize = MultiByteToWideChar(CP_ACP, 0, selectedDll, -1, DllPath, MAX_PATH);

	if (requiredSize == 0 || requiredSize > MAX_PATH) {
		printf("requiredSize != 0 or requiredSize <= MAX_PATH");
	}

	HRESULT result = StringCchPrintfW(Path, MAX_PATH, L"%ls\\System32\\%ls", SharedUserData->NtSystemRoot, DllPath);

	if (FAILED(result)) {
		printf("Failed to StringCchPrint %ls: %08X\n", result);
	}
	// Modified
