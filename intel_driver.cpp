...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size) {
	...
	// MapIoSpace 함수를 호출하여 물리 주소를 IO 공간에 매핑
	...
	// WriteMemory 함수를 호출하여 buffer의 데이터를 매핑된 물리 메모리 위치에 쓰기
	bool result = WriteMemory(device_handle, mapped_physical_memory, buffer, size);

#if defined(DISABLE_OUTPUT) // UnmapIoSpace를 호출하여 매핑된 공간 해제
...
/* https://learn.microsoft.com/ko-kr/windows-hardware/drivers/ddi/wdm/nf-wdm-exallocatepoolwithtag
